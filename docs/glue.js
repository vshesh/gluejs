...
var Glue = (() => {
  ...
  function augment(regex3, captureNames, xSource, xFlags, isInternalOnly) {
    regex3[REGEX_DATA] = {
      captureNames
    };
    if (isInternalOnly) {
    ...
    if (regex3.__proto__) {
      regex3.__proto__ = XRegExp.prototype;
    } else {
      for (const p in XRegExp.prototype) {
        regex3[p] = XRegExp.prototype[p];
      }
    }
    regex3[REGEX_DATA].source = xSource;
    regex3[REGEX_DATA].flags = xFlags ? xFlags.split("").sort().join("") : xFlags;
    return regex3;
  }
  function clipDuplicates(str) {
    return str.replace(/([\s\S])(?=[\s\S]*\1)/g, "");
  }
  function copyRegex(regex3, options) {
    if (!XRegExp.isRegExp(regex3)) {
      throw new TypeError("Type RegExp expected");
    }
    const xData = regex3[REGEX_DATA] || {};
    let flags = getNativeFlags(regex3);
    let flagsToAdd = "";
    let flagsToRemove = "";
    ...
    regex3 = augment(
      new RegExp(options.source || regex3.source, flags),
      hasNamedCapture(regex3) ? xData.captureNames.slice(0) : null,
      xregexpSource,
      xregexpFlags,
  ...
  function getNativeFlags(regex3) {
    return hasFlagsProp ? regex3.flags : (
      // Explicitly using `RegExp.prototype.toString` (rather than e.g. `String` or concatenation
      // with an empty string) allows this to continue working predictably when
      // `XRegExp.proptotype.toString` is overridden
      /\/([a-z]*)$/i.exec(RegExp.prototype.toString.call(regex3))[1]
    );
  }
  function hasNamedCapture(regex3) {
    return !!(regex3[REGEX_DATA] && regex3[REGEX_DATA].captureNames);
  }
  function hex(dec2) {
  ...
  function isQuantifierNext(pattern, pos, flags) {
    ...
    return regex3.test(pattern.slice(pos));
  }
  function isType(value2, type3) {
    return Object.prototype.toString.call(value2) === `[object ${type3}]`;
  }
  function nullThrows(value2) {
    if (value2 == null) {
  ...
  function prepareOptions(value2) {
    const options = {};
    if (isType(value2, "String")) {
      XRegExp.forEach(value2, /[^\s,]+/, (match) => {
        options[match] = true;
      });
      return options;
    }
  ...
  XRegExp._hex = hex;
  XRegExp._pad4 = pad4;
  XRegExp.addToken = (regex3, handler, options) => {
    options = options || {};
  ...
  XRegExp.exec = (str, regex3, pos, sticky) => {
    let cacheKey = "g";
    let addY = false;
    let fakeY = false;
    let match;
    addY = hasNativeY && !!(sticky || regex3.sticky && sticky !== false);
    if (addY) {
      cacheKey += "y";
    } else if (sticky) {
      fakeY = true;
    ...
    regex3[REGEX_DATA] = regex3[REGEX_DATA] || {};
    const r2 = regex3[REGEX_DATA][cacheKey] || (regex3[REGEX_DATA][cacheKey] = copyRegex(regex3, {
      addG: true,
      addY,
      source: fakeY ? `${regex3.source}|()` : void 0,
    ...
    if (regex3.global) {
      regex3.lastIndex = match ? r2.lastIndex : 0;
    }
    return match;
  };
  XRegExp.forEach = (str, regex3, callback) => {
    let pos = 0;
    let i = -1;
    let match;
    while (match = XRegExp.exec(str, regex3, pos)) {
      callback(match, ++i, str, regex3);
      pos = match.index + (match[0].length || 1);
    }
  };
  XRegExp.globalize = (regex3) => copyRegex(regex3, { addG: true });
  ...
  XRegExp.isRegExp = (value2) => Object.prototype.toString.call(value2) === "[object RegExp]";
  XRegExp.match = (str, regex3, scope) => {
    const global = regex3.global && scope !== "one" || scope === "all";
    const cacheKey = (global ? "g" : "") + (regex3.sticky ? "y" : "") || "noGY";
    regex3[REGEX_DATA] = regex3[REGEX_DATA] || {};
    const r2 = regex3[REGEX_DATA][cacheKey] || (regex3[REGEX_DATA][cacheKey] = copyRegex(regex3, {
      addG: !!global,
      removeG: scope === "one",
    ...
    if (regex3.global) {
      regex3.lastIndex = scope === "one" && result ? (
  ...
  XRegExp.matchChain = (str, chain3) => (function recurseChain(values, level) {
    const item = chain3[level].regex ? chain3[level] : { regex: chain3[level] };
    const matches = [];
    ...
    for (const value2 of values) {
      XRegExp.forEach(value2, item.regex, addMatch);
    }
    return level === chain3.length - 1 || !matches.length ? matches : recurseChain(matches, level + 1);
  })([str], 0);
  ...
  XRegExp.replaceEach = (str, replacements) => {
    for (const r2 of replacements) {
      str = XRegExp.replace(str, r2[0], r2[1], r2[2]);
    }
    return str;
  };
  XRegExp.split = (str, separator, limit) => fixed.split.call(nullThrows(str), separator, limit);
  XRegExp.test = (str, regex3, pos, sticky) => !!XRegExp.exec(str, regex3, pos, sticky);
  ...
  fixed.match = function(regex3) {
    if (!XRegExp.isRegExp(regex3)) {
      regex3 = new RegExp(regex3);
    } else if (regex3.global) {
      const result = String.prototype.match.apply(this, arguments);
      regex3.lastIndex = 0;
      return result;
    }
    return fixed.exec.call(regex3, nullThrows(this));
  };
  ...
  var build_default = (XRegExp2) => {
    ...
    function asXRegExp(value2, addFlagX) {
      const flags = addFlagX ? "x" : "";
      return XRegExp2.isRegExp(value2) ? value2[REGEX_DATA2] && value2[REGEX_DATA2].captureNames ? (
        // Don't recompile, to preserve capture names
        value2
      ) : (
        // Recompile as XRegExp
        XRegExp2(value2.source, flags)
      ) : (
        // Compile string as XRegExp
        XRegExp2(value2, flags)
      );
    }
  ...
  var matchrecursive_default = (XRegExp2) => {
    function row(name, value2, start, end) {
      return {
        name,
        value: value2,
        start,
  ...
  var parseOptions = function(aliases, options, value2) {
    let out = {}, key, alias;
    if (options !== void 0) {
  ...
  var write = function(out, key, value2, aliases, unknown) {
  ...
  function getopts_default(argv, opts) {
    let unknown = (opts = opts || {}).unknown, aliases = parseAlias(opts.alias), strings = parseOptions(aliases, opts.string, ""), values = parseDefault(aliases, opts.default), bools = parseOptions(aliases, opts.boolean, false), stopEarly = opts.stopEarly, _ = [], out = { _ }, key, arg, end, match, value2;
    for (let i = 0, len = argv.length; i < len; i++) {
      ...
      if (arg[0] !== "-" || arg === "-") {
        ...
        if (arg[2] === "n" && arg[3] === "o" && arg[4] === "-") {
          ...
          key = arg.slice(2, end);
          value2 = bools[key] !== void 0 || (strings[key] === void 0 ? parseValue(arg.slice(end + 1)) : arg.slice(end + 1));
        } else {
          key = arg.slice(2);
          value2 = bools[key] !== void 0 || (len === i + 1 || argv[i + 1][0] === "-" ? strings[key] === void 0 ? true : "" : strings[key] === void 0 ? parseValue(argv[++i]) : argv[++i]);
        }
        write(out, key, value2, aliases, unknown);
      } else {
        ...
        for (let k = 1; k < end; k++) {
          write(
            out,
            key = arg[k],
            k + 1 < end ? strings[key] === void 0 || arg.substring(k + 1, k = end) + value2 : value2 === "" ? len === i + 1 || argv[i + 1][0] === "-" ? strings[key] === void 0 || "" : bools[key] !== void 0 || (strings[key] === void 0 ? parseValue(argv[++i]) : argv[++i]) : bools[key] !== void 0 || (strings[key] === void 0 ? parseValue(value2) : value2),
            aliases,
            unknown
          );
        }
    ...
    for (let key2 in values) if (out[key2] === void 0) out[key2] = values[key2];
    for (let key2 in bools) if (out[key2] === void 0) out[key2] = false;
    for (let key2 in strings) if (out[key2] === void 0) out[key2] = "";
    return out;
  }
  ...
  function num_groups(regex3) {
    return (new RegExp(regex3.toString() + "|").exec("") || []).length - 1;
  }
  ...
  function forestify1(start, end, tokens2, pos = 0) {
    let forest = [];
    let i = pos;
    let level = 0;
  ...
  var Block = class extends Element {
  ...
  var Inline = class extends Element {
    ...
    validate() {
      ...
      return !(this.display === 0 /* BLOCK */ && !(pattern.startsWith("^") && pattern.endsWith("$")));
    }
  };
  ...
  function inline(a, b, c, d, e, f) {
    ...
    if (a instanceof RegExp && type_default(b) === "Function") return new Inline(a, b, c, e, d, f);
    if (a instanceof RegExp && b === void 0) return (p) => new Inline(a, p, c, e, d, f);
    if (a === void 0) return (regex3, p) => inline(regex3, p);
    return (regex3, p) => inline(regex3, p, a, b, c, d);
  }
  var Patterns = {
    escape: r`(?<!\\)(?:\\\\)*{0}`,
    single_group: r`(?<!\\)(?:\\\\)*{0}(.*?(?<!\\)(?:\\\\)*){1}`,
  ...
  function inline_one(start, end, nest = 0 /* FRAME */, sub = void 0, display = 1 /* INLINE */) {
    ...
    return (p2) => inline(patt, p2, nest, start.slice(0, 1) + end.slice(0, 1), sub, display);
  }
  ...
  function link(designation, nest = 1 /* POST */, sub) {
    const pattern = regex(Patterns.link.replace("{0}", designation));
    return (p) => inline(pattern, p, nest, "()[]" + (designation[0] ?? ""), sub);
  }
  function inline_two(start, mid, end, nest = 1 /* POST */, sub = ["all"]) {
    const pattern = regex(format(Patterns.double_group, start, mid, end));
    return inline(pattern, void 0, nest, "", sub);
  }

  // src/parser.ts
  var regex2 = src_default;
  function isTag(x) {
    const o = x[0];
    return type_default(o) === "String" || o instanceof Array && o.length === 2 && type_default(o[0]) === "String" && type_default(o[1]) === "Object";
  }
  var Registry = class extends Map {
    ...
    resolve(e) {
      if (e instanceof Element) {
        ...
        let v = this.get(e);
        if (v === void 0) throw Error(`Element ${e} not found in registry ${this}`);
        return v;
      }
    }
    inlines() {
    ...
    inline_subscriptions(names, parent) {
      if (includes_default("all", names)) return this.inlines();
      let l = [];
      if (parent && includes_default("inherit", names)) {
        if (includes_default("all", parent.subElements)) return this.inlines();
  ...
  function defrag(tree) {
    return coalesce((n) => value(n)[0] === "<>", tree);
  }
  function parseinline(registry, _element, text, parent) {
    ...
    let escapes = subinline.map((x) => x.escape.split("")).reduce(union_default).join("").replace("[", "\\[").replace("]", "\\]");
    const unescape = escapes.length > 0 ? (t) => t.replace(regex2(`\\\\([${regex2.escape(escapes)}])`), `$1`) : identity_default;
    const patt = ((x) => regex2(x, "sg"))(inlines.map((x) => `(?:${typeof x[0] === "string" ? x[0] : x[0].source})`).join("|"));
    const groupcursors = ((x) => {
      return prepend_default(0)(x[1]);
    })(mapAccum_default((a, b) => [a + b, a + b], 0)(map_default((x) => num_groups(x.regex))(subinline)));
    ...
    for (const match of matches) {
      ...
      const groupind = findIndex_default((x) => x !== void 0)(allgroups);
      const pattind = findIndex_default((x) => x >= groupind, groupcursors);
      const [parser, elem] = inlines[pattind][1];
      const groups = slice_default(groupcursors[pattind], groupcursors[pattind + 1] ?? Infinity, allgroups);
      l.push(slice_default(ind, start, text));
      ind = end;
      switch (elem.nest) {
        case 0 /* FRAME */: {
          l.push(Array.from(splicehtmlmap((t) => parseinline(registry, element, t, parent), parser(groups))));
          break;
        }
        case 3 /* NONE */: {
        ...
        case 1 /* POST */: {
          const inheritQ = includes_default("inherit", elem.sub(Inline));
  ...
  function check(test) {
    return (function(value2) {
      if (test instanceof RegExp) {
        return value2.match(test)?.groups;
      } else {
...
${ast}`);
        ...
        const opts = getopts_default((value(ast).args ?? "").split(" "));
        switch (block2.nest) {
          case 3 /* NONE */: {
          ...
          case 1 /* POST */: {
            ...
            const final = construct(value(parsed), branch(parsed).map((node) => parse(registry, node, block2)));
            return defrag(final);
          }
          case 2 /* SUB */: {
            const lexed = splitblocks1(text);
            const subbed = lexed.map((node, i) => isLeaf(node) ? node : `[|${i}|]`).join("\n\n");
            const parsed = block2.parse(subbed, opts);
            return defrag(Array.from(splicehtmlmap((node) => [parse(registry, !!node.match(/\[\|\d+\|\]/) ? lexed[parseInt(node.slice(2, -2))] : node, block2)], parsed)));
          }
          case 0 /* FRAME */: {
            const parsed = block2.parse(text, opts);
            const final = construct(value(parsed), branch(parsed).map((node) => parse(registry, node, parent ?? block2)));
            return defrag(final);
          }
        }
...
${ast}`);
  ...
  var Tooltip = link("T", 1 /* POST */)(function tooltip(groups) {
    return [["span", { title: groups[1] }], groups[0]];
  });
  var headerRegex = /^(#{1,6})\s(.+)$/;
  var Header = inline(
    headerRegex,
  ...
  var Blockquote = block(1 /* POST */)(function blockquote(text, opts) {
    return [["blockquote", {}], text];
  });
  var Code = block(3 /* NONE */, [])(function code(text, opts) {
    return [["pre", {}], [["code", {}], text]];
  });
  var HorizontalRule = block(3 /* NONE */, [])(function horizontalRule() {
    return [["hr", {}]];
  });
  var SideBySide = block(1 /* POST */)(function sideBySide(text, opts) {
    const lines = text.split("\n");
    const columns = transpose_default(lines.map((l) => l.split("|")));
  ...
  var StandardInline = new Registry().add(
    Underline,
    // must come before Italic
    Bold,
    Italic,
    Monospace,
    Strikethrough,
  ...
  var Katex = block(3 /* NONE */, [])(function katex(text) {
    return [["div.katex", {}], text];
  });
  var Mermaid = block(3 /* NONE */, [])(function mermaid(text) {
  ...
  var UnorderedList = block(1 /* POST */)(function unorderedList(text) {
    const items = text.split("\n").filter((l) => l.startsWith("- ")).map((l) => [["li", {}], l.slice(2)]);
    return [["ul", {}], ...items];
  });
  var OrderedList = block(1 /* POST */)(function orderedList(text) {
    const items = text.split("\n").filter((l) => /^\d+\.\s/.test(l)).map((l) => [["li", {}], l.replace(/^\d+\.\s/, "")]);
    return [["ol", {}], ...items];
  });
  var CriticMarkup = new Registry().add(
    CriticAdd,
    CriticDel,
    CriticHighlight,
    CriticComment,
  ...
  var StandardExtended = new Registry().add(
    // Standard inlines
    Underline,
    Bold,
    Italic,
  ...
  function renderAll(options = {}) {
    ...
    document.querySelectorAll(selector).forEach((script) => {
      ...
      const tag = parse(registry, [{ name: top, args: "" }, text]);
